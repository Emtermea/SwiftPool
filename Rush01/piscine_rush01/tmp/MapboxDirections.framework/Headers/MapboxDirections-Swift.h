...
@interface CLLocation (SWIFT_EXTENSION(MapboxDirections))
@end
...
@interface NSHTTPURLResponse (SWIFT_EXTENSION(MapboxDirections))
@end
...
/// This initializer is intended for use in conjunction with the <code>Directions.urlForCalculating(_:)</code> method.
...
/// The value of this property reflects the time it takes to traverse the entire route under ideal conditions. It is the sum of the <code>expectedTravelTime</code> properties of the route’s legs. You should not assume that the user would travel along the route at a fixed speed. The actual travel time may vary based on the weather, traffic conditions, road construction, and other variables. If the route makes use of a ferry or train, the actual travel time may additionally be subject to the schedules of those services.
...
@interface MBRouteOptions : NSObject <NSSecureCoding>
/// Initializes a route options object for routes between the given waypoints and an optional profile identifier.
/// \param waypoints An array of <code>Waypoint</code> objects representing locations that the route should visit in chronological order. The array should contain at least two waypoints (the source and destination) and at most 25 waypoints.
///
...
/// The route’s shape is delivered in <a href="https://developers.google.com/maps/documentation/utilities/polylinealgorithm">encoded polyline algorithm</a> format.
/// This machine-readable format is considerably more compact than <code>geoJSON</code>.
...
/// You can display this string or read it aloud to the user. The string does not include the distance to or from the maneuver. If you need localized or customized instructions, you can construct them yourself from the step’s other properties or use <a href="https://github.com/Project-OSRM/osrm-text-instructions">osrm-text-instructions</a>.
/// note:
/// If you use MapboxDirections.swift with the Mapbox Directions API, this property is formatted for display to the user. If you use OSRM directly, this property contains a basic string that only includes the maneuver type and direction. Use <a href="https://github.com/Project-OSRM/osrm-text-instructions">osrm-text-instructions</a> to construct a complete instruction string for display.
...
/// This property is only set for roundabout (traffic circle or rotary) maneuvers. For the signposted names associated with a highway exit, use the <code>destinations</code> property.
...
/// This property is typically available in steps leading to or from a freeway or expressway. This property contains route reference codes associated with a road later in the route. If a highway ramp is itself part of a numbered route, its reference code is contained in the <code>codes</code> property.
...
/// This parameter does not affect the route, but you can set the name of a waypoint you pass into a <code>RouteOptions</code> object to help you distinguish one waypoint from another. When you get an array of waypoints back in the completion handler of the <code>Directions.calculate(_:completionHandler:)</code> method.
...
@interface MBWaypoint (SWIFT_EXTENSION(MapboxDirections))
@end
