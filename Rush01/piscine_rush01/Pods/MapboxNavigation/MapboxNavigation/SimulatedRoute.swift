...
class SimulatedRoute : NSObject
{
    ...
    convenience init?(along polyline: [CLLocationCoordinate2D])
    {
        ...
        processedLocations = withShiftedTimestamps(arr: withSpeeds(arr: interpolated(arr: simulatedLocationsWithTurnPenalties())))
    ...
    {
        guard processedLocations.count > 0 else
        {
            return
        }
    ...
    func simulatedLocationsWithTurnPenalties() -> [SimulatedLocation]
    {
        ...
        {
            let currentCoordinate = locations.isEmpty ? polyline.first : polyline[i]
            let course: CLLocationDirection = wrap(floor(polyline[i].direction(to: polyline[i + 1])), min: 0, max: 360)
            let turnPenalty: Double = floor(differenceBetweenAngles(currentCoordinate!.direction(to: polyline[i]), polyline[i].direction(to: polyline[i + 1])))
            let location = SimulatedLocation(coordinate: polyline[i],
                    altitude: 0,
        ...
        locations.append(SimulatedLocation(coordinate: polyline.last!,
                altitude: 0,
                horizontalAccuracy: horizontalAccuracy,
                verticalAccuracy: verticalAccuracy,
    ...
    // Recursively inserts new locations to smooth out gaps longer than 1 second.
     func interpolated(arr : [SimulatedLocation]) -> [SimulatedLocation]
	 {
        var processedLocations = [SimulatedLocation]()

         for i in 0 ..< arr.count - 1
         {
            processedLocations.append(arr[i])

            let speed = (maximumSpeed - minimumSpeed) / 2
            let distance = floor(arr[i].distance(from: arr[i + 1]))
            let timeToTravel: TimeInterval = distance / speed

            if timeToTravel > 1
			{
                let newCoordinate = coordinate(at: speed, fromStartOf: [arr[i].coordinate, arr[i + 1].coordinate])
                let newLocation = SimulatedLocation(coordinate: newCoordinate!,
                        altitude: arr[i].altitude,
                        horizontalAccuracy: horizontalAccuracy,
                        verticalAccuracy: verticalAccuracy,
                        course: wrap(floor(newCoordinate!.direction(to: arr[i + 1].coordinate)), min: 0, max: 360),
                        speed: speed,
                        timestamp: Date())
        ...
        processedLocations.append(arr.last!)
//interpolated(arr: processedLocations)
        return processedLocations.count > arr.count ? interpolated(arr: processedLocations): processedLocations
    }

	func withShiftedTimestamps(arr : [SimulatedLocation]) -> [SimulatedLocation] {
        var processedLocations = [SimulatedLocation]()

        for location in arr {
            if location == arr.first
			{
	...
	func withSpeeds(arr : [SimulatedLocation]) -> [SimulatedLocation]
    {
        return arr.map({$0.withPenalizedSpeed})
    }
